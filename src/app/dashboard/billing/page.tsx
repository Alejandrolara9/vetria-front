...
import { initiateCheckout, PLAN_PRICES, PLAN_LABELS, PlanTarget } from "@/services/wompi";
...
export default function BillingPage() {
  const [loading, setLoading] = useState<PlanTarget | null>(null);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Elige el plan que mejor se adapte a tu clínica. El pago se procesa de forma segura con Wompi.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {(["BASIC", "PRO"] as PlanTarget[]).map((plan) => (
          ...
          >
            ...
            <div>
              <h2 className="text-xl font-bold">{PLAN_LABELS[plan]}</h2>
            ...
            >
              {loading === plan ? "Redirigiendo a Wompi..." : `Contratar ${PLAN_LABELS[plan]}`}
            </button>
          </div>
        ))}
      </div>
