...
const BENEFITS = [
  { icon: "🐾", text: "Historia clínica digital con IA" },
  { icon: "💉", text: "Recordatorios automáticos de vacunas" },
  { icon: "📅", text: "Agenda y citas en un solo lugar" },
  { icon: "👨‍👩‍👧", text: "Portal para dueños de mascotas" },
];

function BrandingPanel() {
  return (
    <div className="hidden lg:flex flex-col justify-between bg-gradient-to-br from-gray-950 via-teal-950 to-slate-900 px-12 py-10 min-h-screen">
      {/* Logo */}
      <Link href="/" className="flex items-center gap-2.5 group w-fit">
        <Image src="/logo.png" alt="Vetria" width={36} height={36} className="rounded-xl shadow-lg" />
        <span className="font-bold text-white text-xl group-hover:text-teal-300 transition-colors">Vetria</span>
      </Link>

      {/* Centro */}
      <div>
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-teal-500/15 border border-teal-400/25 rounded-full text-teal-300 text-xs font-semibold mb-6">
          <span className="w-1.5 h-1.5 bg-teal-400 rounded-full animate-pulse" />
          7 días de prueba gratis · Sin tarjeta
        </div>
        <h1 className="text-3xl font-extrabold text-white leading-tight mb-3">
          Gestiona tu clínica<br />
          <span className="text-teal-400">en menos de 2 minutos</span>
        </h1>
        <p className="text-slate-400 text-sm leading-relaxed mb-8 max-w-xs">
          La plataforma veterinaria diseñada para clínicas colombianas. Todo lo que necesitas, sin complicaciones.
        </p>
      ...
      <p className="text-slate-600 text-xs">
        © {new Date().getFullYear()} Vetria · Hecho en Colombia 🇨🇴
      </p>
    </div>
  );
}
...
export default function RegisterPage() {
  ...
  if (pendingCredential) {
    return (
      <div className="min-h-screen lg:grid lg:grid-cols-2">
        <BrandingPanel />
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-teal-950 to-slate-900 lg:bg-none lg:bg-slate-950 flex items-center justify-center px-6 py-12">
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-2">
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-teal-950 to-slate-900 lg:bg-none lg:bg-slate-950 flex flex-col">

        {/* Navbar móvil (solo visible sin el panel izquierdo) */}
        <nav className="flex lg:hidden items-center justify-between px-6 py-4">
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-10">
          <div className="w-full max-w-md">
            ...
            {/* Badge trial — solo móvil (en desktop está en el panel izquierdo) */}
            ...
            <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-8 shadow-2xl">
              <div className="text-center mb-7">
                <h2 className="text-2xl font-extrabold text-white mb-1">Crea tu clínica</h2>
